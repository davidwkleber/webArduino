...
rectHeight = 30;
  ...
  myPort = new Serial(this, Serial.list()[1], buad);
  ...
  if (inString != null) {
    ...
    if (values.length >= numValues) {
      for (int i=0; i<numValues ; i++) {
        ...
        stroke(255,0,0);
        strokeWeight(1);
        rect(0, partH*i+1, 80, 24);
        fill(255);
        text(int(values[i]), 10, partH*i+12);
        fill(128);
        text(max[i], 40, partH*i+12);
        
        if (Save)
        {
        newRow.setInt(column[i],values[i]);
        recording = true;
        }
        ...
        line(xPos, oldY[i], xPos, partH*(i+1) - mappedVal);
      ...
      if(Save)
      {
        newRow.setInt(column[values.length-1],values[values.length-1]);
        recording=true;
      }
